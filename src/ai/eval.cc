...
GoBanAIEval::DirectionPattern *GoBanAIEval::preset_patterns = nullptr;
int *GoBanAIEval::preset_scores = nullptr;
int preset_patterns_size = 0;
int preset_patterns_skip[6] = {0};

int GoBanAIEval::evalState(const char *gs, int player) {
    // Check parameters
    if (gs == nullptr ||
...
int GoBanAIEval::evalMove(const char *gs, int r, int c, int player) {
    // Check parameters
    if (gs == nullptr ||
        player < 1 || player > 2) return 0;
...
int GoBanAIEval::evalADM(DirectionMeasurement *all_direction_measurement) {
    int score = 0;
...
int GoBanAIEval::matchPattern(DirectionMeasurement *all_direction_measurement,
...
void GoBanAIEval::measureAllDirections(const char *gs,
                                       int r,
                                       int c,
                                       int player,
                                       bool consecutive,
...
void GoBanAIEval::measureDirection(const char *gs,
                                   int r, int c,
                                   int dr, int dc,
                                   int player,
                                   bool consecutive,
    ...
    for (bool reversed = false;; reversed = true) {
        while (true) {
            ...
            // Empty cells
            if (cell == 0) {
                if (space_allowance > 0 && GoBanAIUtils::getCell(gs, cr + dr, cc + dc) == player) {
...
void GoBanAIEval::generatePresetPatterns(DirectionPattern **preset_patterns,
                                         int **preset_scores,
...
int GoBanAIEval::winningPlayer(const char *gs) {
